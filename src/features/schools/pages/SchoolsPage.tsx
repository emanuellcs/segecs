...
import { School, City } from "@/types/database";

// Sort options for schools
const schoolSortOptions = (t: any): SortOption[] => [
  { label: t("schools.fields.name"), column: "name" },
  { label: t("schools.fields.inep"), column: "inep" },
...
const initialFilters: SchoolFilters = {
  cityId: "",
};

// Zod schema for school form
const schoolSchema = (t: any) =>
  z.object({
    name: z.string().min(3, t("schools.validations.nameMin")),
    inep: z.string().optional(),
    city_id: z.string().uuid(t("schools.validations.cityRequired")),
  });
...
type SchoolFormValues = z.infer<ReturnType<typeof schoolSchema>>;
...
export default function SchoolsPage() {
  ...
  const {
    ...
    bulkUpdate,
    isBulkDeleting,
  } = useSupabaseCrud<School>("schools", ["schools"], {
    orderBy: { column: sortColumn, ascending: isSortAsc },
  });

  // Fetch cities for selects
  const { items: cities } = useSupabaseCrud<City>("cities", ["cities"]);
  ...
  const filteredSchools = schools.filter((school) => {
    ...
    const matchesCity = !filters.cityId || school.city_id === filters.cityId;

    return matchesSearch && matchesCity;
  });

  const selection = useSelection(filteredSchools);
  ...
  const {
    ...
    resolver: zodResolver(schoolSchema(t)),
  });
  ...
  const onSubmit = async (data: SchoolFormValues) => {
    try {
      if (selectedSchool) {
        await update({ id: selectedSchool.id, ...data });
        toast.success(t("schools.messages.updateSuccess"));
      } else {
        await create(data);
        toast.success(t("schools.messages.createSuccess"));
      }
      handleCloseForm();
    } catch (error) {
  ...
  const onBulkEditSubmit = async (data: BulkEditValues) => {
    ...
    try {
      ...
      toast.success(t("schools.messages.bulkUpdateSuccess"));
  ...
  const confirmDelete = async () => {
    ...
    try {
      await remove(selectedSchool.id);
      toast.success(t("schools.messages.deleteSuccess"));
  ...
  const confirmBulkDelete = async () => {
    try {
      ...
      toast.success(t("schools.messages.bulkDeleteSuccess"));
  ...
  const pagination = usePagination(filteredSchools);
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 font-medium">{t("schools.subtitle")}</p>
        </div>
        <button
          ...
          pagination.currentItems.map((school) => (
            <div
              ...
              className={cn(
                ...
                selection.isSelected(school.id)
                  ? "border-blue-500 ring-2 ring-blue-50"
                  : "border-gray-100",
              )}
            >
              <button
                onClick={() => selection.toggleSelect(school.id)}
                className={cn(
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium">
                      {t("schools.fields.inep")}: {school.inep || "N/A"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-xs text-gray-600 bg-gray-50 p-2 rounded-lg">
                ...
                <span className="truncate">
                  {cities.find((c: City) => c.id === school.city_id)?.name ||
                    "N/A"}
                </span>
              </div>
                ...
                pagination.currentItems.map((school) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      selection.isSelected(school.id) && "bg-blue-50/50",
                    )}
                  >
                    ...
                    <td className="px-6 py-4 text-gray-600 font-medium">
                      {cities.find((c: City) => c.id === school.city_id)
                        ?.name || "N/A"}
                    </td>
      ...
      <FormModal
        ...
        title={selectedSchool ? t("schools.editTitle") : t("schools.newTitle")}
        description={t("schools.formDescription")}
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="space-y-4">
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input
                  ...
                  placeholder={t("schools.placeholders.schoolNameExample")}
                />
              </div>
              {errors.name && (
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input
                  ...
                  placeholder={t("schools.placeholders.optional")}
                />
              </div>
            </div>
            ...
            <div>
              ...
              >
                <option value="">{t("schools.placeholders.selectCity")}</option>
                {cities.map((c: City) => (
                  <option key={c.id} value={c.id}>
                    {c.name} - {c.state}
                  </option>

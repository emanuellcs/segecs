...
export default function DashboardPage() {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [alunos, estagios, frequencias, vagas, avaliacoes] = await Promise.all([
        ...
        supabase.from('estagios').select('id, status, created_at, aluno_id, data_fim', { count: 'exact' }).order('created_at', { ascending: false }),
      ...
      const semAvaliacaoCount = estagiosAtivosArr.filter(e => !estagiosComAvaliacao.has(e.id)).length;
      ...
      const { data: recentAlunos } = await supabase.from('alunos').select('id, nome').in('id', recentEstagiosIds);
      
      const recentActivities = estagios.data?.slice(0, 5).map(e => ({
        ...e,
  ...
  const handleExportSICE = async () => {
    try {
      ...
      const headers = ['ID Estágio', 'Nome do Aluno', 'Matrícula', 'CPF Aluno', 'Curso', 'Empresa', 'CNPJ', 'Vaga', 'Orientador', 'Supervisor', 'Início', 'Fim', 'CH Total', 'CH Diária', 'Status'];
      const csvData = estagios.map(e => {
        const aluno = e.alunos as any;
        const vaga = e.vagas as any;
        return [
          e.id, aluno?.nome, aluno?.matricula, aluno?.cpf, aluno?.cursos?.nome, 
          vaga?.empresas?.razao_social, vaga?.empresas?.cnpj, vaga?.titulo,
          (e.orientadores as any)?.nome, (e.supervisores as any)?.nome,
          e.data_inicio, e.data_fim, e.carga_horaria_total, e.carga_horaria_diaria, e.status
        ];
      });

      const escapeCSV = (field: any) => `"${String(field || '').replace(/"/g, '""')}"`;
      const csvContent = [headers.join(','), ...csvData.map(row => row.map(escapeCSV).join(','))].join('\n');
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: 'Pendentes', value: stats?.statusDistribution.pendente ?? 0, color: 'bg-amber-400', total: stats?.totalAlunos },
                { label: 'Estagiando', value: stats?.statusDistribution.estagiando ?? 0, color: 'bg-green-500', total: stats?.totalAlunos },
                { label: 'Concluídos', value: stats?.statusDistribution.concluido ?? 0, color: 'bg-blue-600', total: stats?.totalAlunos },
              ].map((item, idx) => {

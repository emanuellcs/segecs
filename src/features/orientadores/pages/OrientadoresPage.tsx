          ...
          pagination.currentItems.map((orientador) => (
            <div
              ...
              className={cn(
                ...
                selection.isSelected(orientador.id)
                  ? "border-blue-500 ring-2 ring-blue-50"
                  : "border-gray-100",
              )}
            >
              <button
                onClick={() => selection.toggleSelect(orientador.id)}
                className={cn(
                ...
                pagination.currentItems.map((orientador) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      selection.isSelected(orientador.id) && "bg-blue-50/50",
                    )}
                  >
      ...
      >
        <form onSubmit={handleSubmitBulk(onBulkEditSubmit)} className="space-y-6">
          <div className="space-y-4">
            <div>
              <label className="text-sm font-bold text-gray-700 ml-1">
                Nova Escola de Vínculo (Opcional)
              </label>
              <select

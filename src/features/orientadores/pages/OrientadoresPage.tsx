...
export default function OrientadoresPage() {
  ...
  const { items: orientadores, isLoading, create, update, remove } = useSupabaseCrud<Orientador>('orientadores', ['orientadores']);
  const { items: escolas } = useSupabaseCrud<any>('escolas', ['escolas']);

  const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<OrientadorFormValues>({
    ...
    <div className="p-6 space-y-6">
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 animate-in fade-in slide-in-from-top-4">
          <h2 className="text-lg font-semibold mb-4">{editingId ? 'Editar Orientador' : 'Novo Orientador'}</h2>
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input {...register('cpf')} className="w-full p-2 border border-gray-300 rounded-lg" placeholder="000.000.000-00" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Escola</label>
              <select {...register('escola_id')} className={cn("w-full p-2 border rounded-lg", errors.escola_id ? "border-red-500" : "border-gray-300")}>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-400">Carregando orientadores...</td></tr>
            ) : orientadores.length === 0 ? (
              <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-400">Nenhum orientador cadastrado.</td></tr>
            ) : (
              orientadores.map((ori) => (
                <tr key={ori.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-2">
                    <button onClick={() => handleEdit(ori)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 size={16} /></button>
                    <button onClick={() => confirm('Excluir?') && remove(ori.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size={16} /></button>
                  </td>
                </tr>
              ))
            )}

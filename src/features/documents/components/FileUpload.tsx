...
export default function FileUpload({
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      toast.success(
        t("documents.messages.uploadSuccess", "File uploaded successfully!"),
      );
    } catch (error) {
      const message =
        error instanceof Error ? error.message : t("errors.unexpected");
      toast.error(
        t("documents.messages.uploadError", "Upload error: ") + message,
      );
    } finally {
      setUploading(false);
    }
          ...
          {uploading ? (
            ...
            <>
              ...
              <p className="text-sm text-gray-500">
                {t(
                  "documents.placeholders.dropzone",
                  "Click or drag to send (PDF/IMG)",
                )}
              </p>
            </>
          )}

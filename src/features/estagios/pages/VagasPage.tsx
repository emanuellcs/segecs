...
const vagaSchema = z.object({
  ...
  status: z.enum(['aberta', 'preenchida', 'cancelada']).default('aberta'),
});
...
export default function VagasPage() {
  ...
  const { items: vagas, isLoading, create, update, remove } = useSupabaseCrud<Vaga>('vagas', ['vagas']);
  const { items: empresas } = useSupabaseCrud<any>('empresas', ['empresas']);
  const { items: cursos } = useSupabaseCrud<any>('cursos', ['cursos']);

  const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<VagaFormValues>({
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 animate-in fade-in slide-in-from-top-4">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input {...register('titulo')} className={cn("w-full p-2 border rounded-lg", errors.titulo ? "border-red-500" : "border-gray-300")} placeholder="Ex: Estágio em Desenvolvimento Web" />
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Curso Destinado</label>
              <select {...register('curso_id')} className={cn("w-full p-2 border rounded-lg", errors.curso_id ? "border-red-500" : "border-gray-300")}>
            ...
            <div>
              ...
              <input type="number" {...register('quantidade', { valueAsNumber: true })} className="w-full p-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea {...register('descricao')} className="w-full p-2 border border-gray-300 rounded-lg h-24" placeholder="O que o estagiário irá fazer?" />
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">Carregando vagas...</td></tr>
            ) : vagas.length === 0 ? (
              <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">Nenhuma vaga cadastrada.</td></tr>
            ) : (
              vagas.map((vaga) => (
                <tr key={vaga.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    <div className="text-gray-700 font-medium">{vaga.titulo}</div>
                    <div className="text-xs text-gray-400">{empresas.find(e => e.id === vaga.empresa_id)?.razao_social}</div>
                  </td>
                  <td className="px-6 py-4 text-gray-600 text-sm">
                    {cursos.find(c => c.id === vaga.curso_id)?.nome || 'N/A'}
                  </td>
                  <td className="px-6 py-4 text-gray-600 text-center md:text-left">{vaga.quantidade}</td>
                  <td className="px-6 py-4">
                    <span className={cn(
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-2">
                    <button onClick={() => handleEdit(vaga)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 size={16} /></button>
                    <button onClick={() => confirm('Excluir?') && remove(vaga.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size={16} /></button>
                  </td>
                </tr>
              ))
            )}

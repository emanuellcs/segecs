...
export default function ProjetosSociaisPage() {
  ...
  const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<ProjetoFormValues>({
    ...
    <div className="p-6 space-y-6">
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 animate-in fade-in slide-in-from-top-4">
          <h2 className="text-lg font-semibold mb-4">{editingId ? 'Editar Projeto' : 'Novo Projeto'}</h2>
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Título do Projeto</label>
              <input {...register('titulo')} className={cn("w-full p-2 border rounded-lg", errors.titulo ? "border-red-500" : "border-gray-300")} placeholder="Ex: Manutenção de computadores na APAE" />
            ...
            <div>
              ...
              <input type="number" {...register('horas_estimadas', { valueAsNumber: true })} className="w-full p-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Data de Execução</label>
              <input type="date" {...register('data_execucao')} className="w-full p-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
            ...
            <div className="md:col-span-3">
              ...
              <textarea {...register('descricao')} className="w-full p-2 border border-gray-300 rounded-lg h-24" placeholder="O que será/foi realizado no projeto?" />
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">Carregando projetos...</td></tr>
            ) : projetos.length === 0 ? (
              <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">Nenhum projeto social registrado.</td></tr>
            ) : (
              projetos.map((proj) => (
                <tr key={proj.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-2">
                    <button onClick={() => handleEdit(proj)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-all"><Edit2 size={16} /></button>
                    <button onClick={() => confirm('Excluir?') && remove(proj.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-all"><Trash2 size={16} /></button>
                  </td>
                </tr>
              ))
            )}

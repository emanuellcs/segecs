...
export default function EscolasPage() {
  ...
  const { items: escolas, isLoading, create, update, remove } = useSupabaseCrud<Escola>('escolas', ['escolas']);
  const { items: cidades } = useSupabaseCrud<any>('cidades', ['cidades']);

  const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<EscolaFormValues>({
    ...
    <div className="p-6 space-y-6">
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 animate-in fade-in slide-in-from-top-4">
          <h2 className="text-lg font-semibold mb-4">{editingId ? 'Editar Escola' : 'Nova Escola'}</h2>
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {errors.cidade_id && <p className="text-red-500 text-xs mt-1">{errors.cidade_id.message}</p>}
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-400">Carregando escolas...</td>
              </tr>
            ) : escolas.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-400">Nenhuma escola cadastrada.</td>
              </tr>
            ) : (
              escolas.map((escola) => (

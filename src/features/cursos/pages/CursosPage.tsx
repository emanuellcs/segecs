...
const cursoSchema = z.object({
  ...
  carga_horaria_obrigatoria: z.number().min(1, "Mínimo 1 hora"),
});

const bulkEditSchema = z.object({
  escola_id: z.string().uuid().optional(),
  nivel_id: z.string().uuid().optional(),
  carga_horaria_obrigatoria: z.number().min(1).optional(),
});
...
interface Curso {
  ...
  escola_id: string;
  nivel_id: string;
  carga_horaria_obrigatoria: number;
  created_at: string;
}
...
export default function CursosPage() {
  ...
  const handleEdit = (curso: Curso) => {
    ...
    reset({
      ...
      carga_horaria_obrigatoria: curso.carga_horaria_obrigatoria,
    });
    setIsFormOpen(true);
  };
          ...
          pagination.currentItems.map((curso) => (
            <div
              ...
              className={cn(
                ...
                selection.isSelected(curso.id)
                  ? "border-blue-500 ring-2 ring-blue-50"
                  : "border-gray-100",
              )}
            >
              <button
                onClick={() => selection.toggleSelect(curso.id)}
                className={cn(
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium">
                      {niveis.find((n: any) => n.id === curso.nivel_id)
                        ?.descricao || "N/A"}
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-gray-600 bg-gray-50 p-2 rounded-lg">
                  ...
                  <span className="truncate">
                    {escolas.find((e: any) => e.id === curso.escola_id)?.nome ||
                      "N/A"}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-xs text-blue-700 bg-blue-50 p-2 rounded-lg font-bold">
                  <Clock size={14} />
                  <span>
                    {curso.carga_horaria_obrigatoria}h totais de estágio
                  </span>
                </div>
              </div>
                ...
                pagination.currentItems.map((curso) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      selection.isSelected(curso.id) && "bg-blue-50/50",
                    )}
                  >
                    ...
                    <td className="px-6 py-4 text-gray-600 font-medium">
                      {escolas.find((e: any) => e.id === curso.escola_id)
                        ?.nome || "N/A"}
                    </td>
                    <td className="px-6 py-4 text-gray-600 font-medium">
                      {niveis.find((n: any) => n.id === curso.nivel_id)
                    ...
                    <td className="px-6 py-4 text-blue-600 font-black text-sm">
                      {curso.carga_horaria_obrigatoria}h
                    </td>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input
                  type="number"
                  {...register("carga_horaria_obrigatoria", {
                    valueAsNumber: true,
                  })}
                  className={cn(
                    "w-full pl-10 pr-3 py-2.5 rounded-lg border text-sm focus:ring-2 outline-none transition-all",
                    errors.carga_horaria_obrigatoria
                      ? "border-red-500 focus:ring-red-200"
                      : "border-gray-200 focus:ring-blue-100 focus:border-blue-500",
                  )}
                />
              ...
              {errors.carga_horaria_obrigatoria && (
                <p className="text-[11px] font-bold text-red-500 mt-1 ml-1">
                  {errors.carga_horaria_obrigatoria.message}
                </p>
              )}
            </div>
          </div>
      ...
      >
        <form onSubmit={handleSubmitBulk(onBulkEditSubmit)} className="space-y-6">
          <div className="space-y-4">
            <div>
              <label className="text-sm font-bold text-gray-700 ml-1">
                Nova Escola (Opcional)
              </label>
              <select
                {...registerBulk("escola_id")}
            ...
            <div>
              <label className="text-sm font-bold text-gray-700 ml-1">
                Nova CH Obrigatória (Opcional)
              </label>
              <input
                type="number"
                {...registerBulk("carga_horaria_obrigatoria", {
                  valueAsNumber: true,
                })}

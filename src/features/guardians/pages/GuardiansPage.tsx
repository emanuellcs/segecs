...
// Sort options for guardians
const guardianSortOptions = (t: any): SortOption[] => [
  { label: t("guardians.fields.name"), column: "name" },
  { label: t("guardians.fields.cpf"), column: "cpf" },
  { label: t("common.createdAt"), column: "created_at" },
];

// Zod schema for guardian form
const guardianSchema = (t: any) =>
  z.object({
    name: z.string().min(3, t("guardians.validations.nameMin")),
    cpf: z.string().min(14, t("guardians.validations.cpfInvalid")),
    phone: z.string().min(14, t("guardians.validations.phoneInvalid")),
  });

type GuardianFormValues = z.infer<ReturnType<typeof guardianSchema>>;
...
export default function GuardiansPage() {
  ...
  const {
    ...
    bulkRemove,
    isBulkDeleting,
  } = useSupabaseCrud<Guardian>("guardians", ["guardians"], {
  ...
  const {
    ...
    resolver: zodResolver(guardianSchema(t)),
  });

  const onSubmit = async (data: GuardianFormValues) => {
    try {
      if (selectedGuardian) {
        await update({ id: selectedGuardian.id, ...data });
        toast.success(t("guardians.messages.updateSuccess"));
      } else {
        await create(data);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await remove(selectedGuardian.id);
      toast.success(t("guardians.messages.deleteSuccess"));
  ...
  const confirmBulkDelete = async () => {
    try {
      ...
      toast.success(t("guardians.messages.bulkDeleteSuccess"));
  ...
  const selection = useSelection(filteredGuardians);
  const pagination = usePagination(filteredGuardians);
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl font-black text-blue-900 flex items-center gap-2">
            <Users className="text-blue-600" size={28} /> {t("guardians.title")}
          </h1>
          <p className="text-gray-500 font-medium">{t("guardians.subtitle")}</p>
        </div>
        <button
      ...
      >
        {filteredGuardians.length === 0 ? (
          ...
          pagination.currentItems.map((guardian) => (
            <div
              key={guardian.id}
              className={cn(
                "bg-white p-5 rounded-2xl shadow-sm border transition-all relative group space-y-4",
                selection.isSelected(guardian.id)
                  ? "border-blue-500 ring-2 ring-blue-50"
                  : "border-gray-100",
              )}
            >
              <button
                onClick={() => selection.toggleSelect(guardian.id)}
                className={cn(
            ...
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest">
                  {t("guardians.fields.name")}
                </th>
                <th className="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest">
                  {t("guardians.fields.cpf")}
                </th>
                <th className="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest">
                ...
                pagination.currentItems.map((guardian) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      selection.isSelected(guardian.id) && "bg-blue-50/50",
                    )}
                  >
      ...
      <FormModal
        ...
        title={
          selectedGuardian ? t("guardians.editTitle") : t("guardians.newTitle")
        }
        description={t("guardians.formDescription")}
      >
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input
                  ...
                  placeholder={t("guardians.placeholders.guardianNameExample")}
                />
              </div>
              {errors.name && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <InputMask
                    mask="phone"
                    label={t("guardians.fields.phone")}
                    value={field.value || ""}
                    onChange={field.onChange}
      ...
      <ConfirmDeleteModal
        ...
        itemName={selectedGuardian?.name}
      />
